...
import myChat.model.ValidationCheck;

import myChat.controller.ChatSystem;
import myChat.controller.AdminAuthentication;
import myChat.controller.DisplayImageAwt;
...
public class ChatyManagement extends ValidationCheck
{
 ...
 public static void getAdminDetails() {
  ...
  public void startChat(String userName){
      ...
      String message=" ";
      while(!message.equals("CCC")){
          message = input.nextLine();
          senderMessage.add(message);
      }
      senderMessage.remove("CCC");
     /* String senderMessage = input.nextLine();
      while(!senderMessage.equals("send")){
          senderMessage = input.nextLine();
      }*/
     ...
     if (senderId != -1 && receiverId != -1) {
        if(chatyManagement.chatSystem.isFollowerAlreadyExists(senderId, receiverId)){
...
public long viewNotifications(String userName){
   long userId = chatyManagement.chatSystem.findUserIdByUsername(userName);
   long notificationId = chatyManagement.chatSystem.viewNotifications(userId);
   return notificationId;
}

public void acceptRequest(String userName,int notificationIdTOUser){
    System.out.println("Enter the notification ID to accept:");
    int notificationId = input.nextInt();
    while(notificationIdTOUser !=notificationId){
        System.out.println("Notifition id not match.Enter again correctly:");
        notificationId = input.nextInt();
    }
    input.nextLine(); 
